...
@app.route('/')
def dashboard():
    engine = create_engine(DATABASE_URL)  # Connexion à la base de données
    try:
        with engine.connect() as conn:
            # 1. Chiffre d'affaires des 30 derniers jours (sans virgule)
            ca_30_jours = conn.execute(text("""
                SELECT 
                    COALESCE(CAST(SUM(quantite_vendue * prix_achat) AS INTEGER), 0) AS ca_30_jours
                FROM "Ventes"
                WHERE date_vente >= CURRENT_DATE - INTERVAL '30 days';
            """)).scalar() or 0

            # 2. Évolution du chiffre d'affaires (comparaison avec la même période l'année dernière)
            evolution_data = conn.execute(text("""
                WITH ca_courant AS (
                    SELECT 
                        COALESCE(SUM(quantite_vendue * prix_achat), 0) AS ca_30_jours_courant
                    FROM "Ventes"
                    WHERE date_vente >= CURRENT_DATE - INTERVAL '30 days'
                ),
                ca_annee_precedente AS (
                    SELECT 
                        COALESCE(SUM(quantite_vendue * prix_achat), 0) AS ca_30_jours_precedent
                    FROM "Ventes"
                    WHERE date_vente BETWEEN (CURRENT_DATE - INTERVAL '1 year' - INTERVAL '30 days')
                                      AND (CURRENT_DATE - INTERVAL '1 year')
                )
                SELECT 
                    ca_courant.ca_30_jours_courant,
                    ca_annee_precedente.ca_30_jours_precedent,
                    CASE 
                        WHEN ca_annee_precedente.ca_30_jours_precedent = 0 THEN NULL
                        ELSE CAST(
                            ROUND(
                                ((ca_courant.ca_30_jours_courant - ca_annee_precedente.ca_30_jours_precedent) 
                                / NULLIF(ca_annee_precedente.ca_30_jours_precedent, 0)::NUMERIC * 100)::NUMERIC, 2
                            ) AS INTEGER)
                    END AS evolution
            ...
            ca_30_jours_courant = evolution_data[0] or 0
            ca_30_jours_precedent = evolution_data[1] or 0
            evolution = evolution_data[2] or 0  # Évolution du chiffre d'affaires

            # 3. Top 10 des produits les plus vendus (30 derniers jours)
            top_products = conn.execute(text("""
            SELECT 
                v.code_article,
                p.nom_produit,
                CAST(SUM(v.quantite_vendue) AS INTEGER) AS quantite_totale,  -- Arrondi sans virgule
            ...
            # 4. Top 5 des représentants ayant fait le plus de chiffre (30 derniers jours)
        ...
        return render_template(
            'dashboard.html', 
            ca_mois=ca_30_jours_courant,  # Chiffre d'affaires des 30 derniers jours (sans virgule)
            evolution=evolution,  # Evolution % par rapport à la même période l'année dernière
